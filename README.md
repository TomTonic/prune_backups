...
### Package Repository (auto-updating)

`prune_backups` is available through the [acmelab package repository](https://pkg.acmelab.de), a signed `apt`/`dnf`/`zypper`/`pacman`/`apk` repository built directly from this project's own GitHub Releases (see [TomTonic/pkg-repo](https://github.com/TomTonic/pkg-repo) for how it's built and why only the index, not every package, is signed). Once added, `apt upgrade`/`dnf upgrade`/`pacman -Syu`/`apk upgrade` pick up new releases automatically - no manual downloads needed.
