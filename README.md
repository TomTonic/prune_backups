...
`prune_backups` is a single, dependency-free binary - no runtime, no libraries. On Linux it's available through a signed, auto-updating `apt`/`dnf`/`zypper`/`pacman`/`apk` repository; it also runs great on Windows and macOS.

**→ See [INSTALL.md](INSTALL.md) for setup instructions for your platform.**
